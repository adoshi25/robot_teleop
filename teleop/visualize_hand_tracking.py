...
# ---------------------------------------------------------------------------
# Config (loaded from teleop/config.yaml, fallback to defaults)
# ---------------------------------------------------------------------------
...
_HT = _CFG.get("hand_tracking", {})
_ARM_LIMITS = _CFG.get("limits", {}).get("arm", {})


def _cfg(key, default):
    return _HT.get(key, default)


HAND_TRACKER_URL = _cfg("hand_tracker_url", "http://localhost:9002/get_hand_data")
FRAME_POST_URL = _cfg("frame_post_url", "http://localhost:9002/mujoco_frame")
FRAME_POST_HZ = _cfg("frame_post_hz", 15)
OFFSCREEN_WIDTH = _cfg("offscreen_width", 640)
OFFSCREEN_HEIGHT = _cfg("offscreen_height", 480)
DEFAULT_MJCF = _REPO_ROOT / _cfg("default_mjcf", "teleop/robots/tesollo_hand/robot_scene_combined.xml")
CONTROL_HZ = _cfg("control_hz", 60)
WARMUP_SECONDS = _cfg("warmup_seconds", 10)
MAX_JOINT_DELTA = _cfg("max_joint_delta", 1)
WRIST_POSE_SENSITIVITY = _cfg("wrist_pose_sensitivity", 1.0)
THUMB_OPP_GAIN = _cfg("thumb_opp_gain", 1.5)
...
ROBOT_SITES = _cfg(
    ...
    {
        "left": {
            "wrist": "left_palm_site", "thumb-tip": "left_thumb_tip_site",
            "index-finger-tip": "left_index_tip_site", "middle-finger-tip": "left_middle_tip_site",
            "ring-finger-tip": "left_ring_tip_site", "pinky-finger-tip": "left_pinky_tip_site",
        },
        "right": {
            "wrist": "right_palm_site", "thumb-tip": "right_thumb_tip_site",
            "index-finger-tip": "right_index_tip_site", "middle-finger-tip": "right_middle_tip_site",
            "ring-finger-tip": "right_ring_tip_site", "pinky-finger-tip": "right_pinky_tip_site",
        },
    },
)

FINGER_KEYPOINTS = _cfg(
    "finger_keypoints",
    {
...
FINGER_JOINT_NAMES = _cfg(
    "finger_joint_names",
    {
        "left": {"thumb": ["lj_dg_1_1", "lj_dg_1_2", "lj_dg_1_3", "lj_dg_1_4"], "index": ["lj_dg_2_1", "lj_dg_2_2", "lj_dg_2_3", "lj_dg_2_4"], "middle": ["lj_dg_3_1", "lj_dg_3_2", "lj_dg_3_3", "lj_dg_3_4"], "ring": ["lj_dg_4_1", "lj_dg_4_2", "lj_dg_4_3", "lj_dg_4_4"], "pinky": ["lj_dg_5_1", "lj_dg_5_2", "lj_dg_5_3", "lj_dg_5_4"]},
        "right": {"thumb": ["rj_dg_1_1", "rj_dg_1_2", "rj_dg_1_3", "rj_dg_1_4"], "index": ["rj_dg_2_1", "rj_dg_2_2", "rj_dg_2_3", "rj_dg_2_4"], "middle": ["rj_dg_3_1", "rj_dg_3_2", "rj_dg_3_3", "rj_dg_3_4"], "ring": ["rj_dg_4_1", "rj_dg_4_2", "rj_dg_4_3", "rj_dg_4_4"], "pinky": ["rj_dg_5_1", "rj_dg_5_2", "rj_dg_5_3", "rj_dg_5_4"]},
    },
)

FINGER_ANGLE_TO_JOINT = _cfg(
    "finger_angle_to_joint",
    {"thumb": {0: 2, 1: 3}, "index": {0: 1, 1: 2, 2: 3}, "middle": {0: 1, 1: 2, 2: 3}, "ring": {0: 1, 1: 2, 2: 3}, "pinky": {1: 2, 2: 3}},
)

_ABD_FINGERS_RAW = _cfg(
    "abd_fingers",
    [
...
_ABD_FINGERS = [tuple(row) for row in _ABD_FINGERS_RAW]

ARM_JOINT_VEL_LIMITS = np.array(_ARM_LIMITS.get("joint_vel", [2.075, 2.075, 2.075, 2.075, 2.8, 2.8, 2.8]))
            ...
            if hand_data and self.offsets:
                for side in ("left", "right"):
                    ...
                    if wrist_pose is not None:
                        wrist_pose = self._apply_wrist_sensitivity(wrist_pose, side)
